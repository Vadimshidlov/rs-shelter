...
function goTabletSixPets(arr) {
    ...
    return boolArr.includes(0) ? goTabletSixPets(getArrayOfPageArray(petCards, count)) : arr;
}
...
function goMobileThreePets(arr) {
    ...
    const res = [];
    for (let i = 0; i < arr.length; i += 6) {
        if (arr[i] !== arr[i + 1] && arr[i] !== arr[i + 2] && arr[i + 1] !== arr[i] && arr[i + 1] !== arr[i + 2]) {
            res.push(1);
        } else {
            res.push(0);
        }
    ...
    return res.includes(0) ? goMobileThreePets(getArrayOfPageArray(petCards)) : arr;
}
    ...
    <div class="modal-window__wrapper">
       <div class="modal-window__content">
          ...
          <div class="modal-window__image">
             <img src=${dataModalWindowPets[id].img} alt="" />
          </div>
